...
export const cashbackQueueOptionsProducer: RmqOptions = {
...
export const cashbackQueueOptionsConsumer: RmqOptions = {
  transport: Transport.RMQ,
  options: {
    ...option,
...
const cashbackQueueProvider = {
  provide: QUEUES.CASHBACK, //là tên hoặc token để nhận diện provider. Token này sẽ được sử dụng khi bạn muốn inject provider vào một thành phần khác.
  useFactory: () => {
    ...
    return ClientProxyFactory.create(cashbackQueueOptionsConsumer); //được sử dụng để tạo ra client proxy.
  },
};

export default cashbackQueueProvider;
...
/*
  ...
  - Sau khi thêm vào module, bạn có thể inject nó vào bất kỳ thành phần nào bằng cách sử dụng token QUEUES.wallet:
*/

/*
  1. walletQueueProvider là một custom provider được định nghĩa bằng factory function để tạo ra một client proxy cho hệ thống hàng đợi.
