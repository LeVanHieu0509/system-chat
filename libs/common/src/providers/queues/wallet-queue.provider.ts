...
export const walletQueueOptionsProducer: RmqOptions = {
...
export const walletQueueOptionsConsumer: RmqOptions = {
  transport: Transport.RMQ,
  options: {
    ...option,
...
const walletQueueProvider = {
  provide: QUEUES.WALLET, //là tên hoặc token để nhận diện provider. Token này sẽ được sử dụng khi bạn muốn inject provider vào một thành phần khác.
  useFactory: () => {
    ...
    return ClientProxyFactory.create(walletQueueOptionsConsumer); //được sử dụng để tạo ra client proxy.
  },
};

export default walletQueueProvider;
